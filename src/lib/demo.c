...
int loadDemoData(NVGcontext* vg, DemoData* data)
{
	...
	for (i = 0; i < 4; i++) {
		char file[128];
		snprintf(file, 128, "assets/images/image%d.jpg", i+1);
	...
	data->fontIcons = nvgCreateFont(vg, "icons", "assets/fonts/entypo.ttf");
	if (data->fontIcons == -1) {
		printf("Could not add font icons.\n");
		return -1;
	}
	data->fontNormal = nvgCreateFont(vg, "sans", "assets/fonts/Roboto-Regular.ttf");
	if (data->fontNormal == -1) {
		printf("Could not add font italic.\n");
		return -1;
	}
	data->fontBold = nvgCreateFont(vg, "sans-bold", "assets/fonts/Roboto-Bold.ttf");
	if (data->fontBold == -1) {
		printf("Could not add font bold.\n");
		return -1;
	}
	data->fontEmoji = nvgCreateFont(vg, "emoji", "assets/fonts/NotoEmoji-Regular.ttf");
	if (data->fontEmoji == -1) {
